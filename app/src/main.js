...
async function getData(params) {
    try{
        ...
        //get data from api
        const response = await fetch(`https://api.currencybeacon.com/v1/latest?api_key=${apiKey}`)
...
async function convert(){
    ...
    try{
        //get data from api
        const response = await fetch(`https://api.currencybeacon.com/v1/convert?api_key=${apiKey}&from=${select1.value}&to=${select2.value}&amount=${current.value}`)
        if(response.status != 200){
            throw new Error(response)
        }else{
            //converts response into json we can use
